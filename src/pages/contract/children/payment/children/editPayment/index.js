...
function EditPayment(props) {
    const {title, visible, setVisible, width, code, dataForm} = props;
    ...
    const [payment, setPayment] = useState({
        ...
        "contract_code": code,
        "payment": {
            "storage_material_ids": dataForm['storage_material_ids'],
            "entry_ids": dataForm['entry_ids'],
            "payment_id": dataForm['payment_id'],
            "payment_name": dataForm['payment_name'],
            "bill_number": dataForm['bill_number'],
            "bill_type": dataForm['bill_type'],
            "amount_money": dataForm['amount_money'],
            "amount_tax": dataForm['amount_tax'],
            "amount_total": dataForm['amount_total'],
            "bank": dataForm['bank'],
            "bank_account": dataForm['bank_account'],
            "invoicer": dataForm['invoicer'],
            "invoice_date": dataForm['invoice_date'],
            "payment_ratio": dataForm['payment_ratio'],
    ...
    const queryTable = (values) => {
        // 格式化日期
        values['invoice_date'] = values['invoice_date'].format('YYYYMMDD');
            ...
            <div className="contract-search">
                <Form name="horizontal_login"
                    ...
                    <Form.Item name="storage_material_ids" label="入供应商成品库："
                               initialValue={payment['payment']['storage_material_ids']}>
                        <Select
                            mode="multiple"
                            allowClear
                            style={{width: '100%'}}
                    ...
                    <Form.Item name="entry_ids" label="入库单：" initialValue={payment['payment']['entry_ids']}>
                        <Select
                            mode="multiple"
                            allowClear
                            style={{width: '100%'}}
                    ...
                    <Form.Item name="payment_id"
                               ...
                               initialValue={payment['payment']['payment_id']}
                               label="款项ID：">
                        <Input placeholder="请输入款项ID" allowClear/>
                    </Form.Item>
                    <Form.Item name="payment_name"
                               label="款项名称："
                               initialValue={payment['payment']['payment_name']}
                               rules={[{required: true, message: '请输入款项名称'}]}>
                        <Input placeholder="请输入款项名称" allowClear/>
                    </Form.Item>
                    <Form.Item name="bill_number"
                               label="票据号码："
                               initialValue={payment['payment']['bill_number']}
                               rules={[{required: true, message: '请输入票据号码'}]}>
                        <Input placeholder="请输入票据号码"
                               allowClear/>
                    </Form.Item>
                    <Form.Item name="bill_type"
                               label="票据类型："
                               initialValue={payment['payment']['bill_type']}>
                        <Input placeholder="请输入票据类型"
                               allowClear/>
                    </Form.Item>
                    <Form.Item name="amount_money"
                               label="金额："
                               rules={[{required: true, message: '请输入金额'}]}
                               initialValue={payment['payment']['amount_money']}>
                        <Input placeholder="请输入金额"
                               allowClear/>
                    </Form.Item>
                    <Form.Item name="amount_tax"
                               label="税额："
                               initialValue={payment['payment']['amount_tax']}
                               rules={[{required: true, message: '请输入税额'}]}>
                        <Input placeholder="请输入税额"
                               allowClear/>
                    </Form.Item>
                    <Form.Item name="amount_total"
                               label="价税合计："
                               initialValue={payment['payment']['amount_total']}
                               rules={[{required: true, message: '请输入价税合计'}]}>
                        <Input placeholder="请输入价税合计"
                               allowClear/>
                    </Form.Item>
                    <Form.Item name="bank"
                               label="开户行："
                               initialValue={payment['payment']['bank']}
                               rules={[{required: true, message: '请输入开户行'}]}>
                        <Input placeholder="请输入开户行"
                               defaultValue={payment['payment']['bank']} allowClear/>
                    </Form.Item>
                    <Form.Item name="bank_account"
                               initialValue={payment['payment']['bank_account']}
                               label="开户行账号：">
                        <Input placeholder="请输入开户行账号"
                               allowClear/>
                    </Form.Item>
                    <Form.Item name="invoicer"
                               label="开票单位："
                               initialValue={payment['payment']['invoicer']}
                    ...
                    <Form.Item name="invoice_date"
                               label="发票日期："
                               initialValue={moment(payment['payment']['invoice_date'], 'YYYYMMDD')}
                               rules={[{required: true, message: '请选择发票日期'}]}>
                        <DatePicker placeholder="请选择发票日期"
                                    allowClear/>
                    </Form.Item>
                    <Form.Item name="payment_ratio"
                               label="支付比例："
                               initialValue={payment['payment']['payment_ratio']}
                               rules={[{required: true, message: '请输入支付比例'}]}>
                        <Input placeholder="请输入支付比例"
                               allowClear/>
                    </Form.Item>
                    <Form.Item name="payment_type"
                               label="款项类型："
                               initialValue={payment['payment']['payment_type']}
